...
const categoryDescriptions = {
  developer:
    "Commands related to Yui's core process.\n\nUsage restricted to bot-owners only.",
  fun:
    "Commands that are fun in some way, such as being a game, or being funny.",
  information:
    "Commands related to Yui, such as statistics, or general information.\n\nAlso includes some commands to gather general information related to things such as users or servers.",
  search:
    "Search commands. Usually this means they query an outside API or service, but it can vary.",
...
module.exports = class Help extends Command {
  constructor(client) {
    super(client, {
      name: "help",
      description:
        "Lists all of my commands or information related to a singularity",
      usage: "<?command|category|args>",
    });
  }
  async run(message, args) {
    const prefix =
      (await this.client.prefixes.get(message.guild.id)) ||
      ...
      const embed = new MessageEmbed()
        ...
        .setDescription(
          this.client.categories
            .map((c) => `≫ ${this.client.util.firstUpperCase(c)}`)
            .concat(["≫ All"])
            .sort()
            .join("\n")
        )
        .setFooter(
          `Use ${prefix}help <command> to see more information about a category, and the commands that fall into it.`
        );
    ...
    if (
      (!this.client.categories.includes(
        this.client.util.firstUpperCase(args[0])
      ) &&
      ...
      (!this.client.categories.includes(
        ...
        this.client.commands.find(
          (cmd) => cmd.conf.aliases && cmd.conf.aliases.includes(name)
        ))
    ) {
      result =
        this.client.commands.get(name) ||
      ...
      if (result.conf.aliases)
        description += `\nAliases: ${result.conf.aliases.join(", ")}`;
      if (result.help.description)
        description += `\nDescription: ${result.help.description}`;
      if (result.help.usage)
      ...
      this.client.categories.includes(this.client.util.firstUpperCase(args[0]))
    ) {
      ...
      const embed = new MessageEmbed()
        ...
        .setColor(colors.embeds)
        .setFooter(
          `Use ${prefix}help [COMMAND] to see more information about a command.`
        );
