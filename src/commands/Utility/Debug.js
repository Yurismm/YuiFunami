...
module.exports = class Debug extends Command {
  ...
  async run(message, args) {
    ...
    this.client.util.getDirectories(join(__dirname, "..")).forEach((d) => {
      let commands = readdirSync(join(__dirname, "..", d))
    ...
    const embed = new MessageEmbed()
      ...
      .addField(
        "Commands:",
        `Loaded: ${this.client.commands.size}\nAvailable: ${availableCommands}`
      )
      .addField(
        "Permissions:",
        `Add Reactions: ${message.guild.me.hasPermission(
          "ADD_REACTIONS"
        )}\nManage Messages: ${message.guild.me.hasPermission(
          "MANAGE_MESSAGES"
        )}\nRead Messages: ${message.guild.me.hasPermission(
          "VIEW_CHANNEL"
        )}\nRead Message History: ${message.guild.me.hasPermission(
