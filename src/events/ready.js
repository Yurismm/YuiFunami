...
module.exports = class {
  ...
  async execute() {
    ...
    this.client.logger.info(
      `Defaultprefix set to: ${this.client.config.defaultSettings.prefix}`
    );
    this.client.logger.info(
      `${magenta(this.client.commands.size)} commands loaded`
    );
    this.client.logger.info(`Program version: ${blue("v" + version)}`);
    this.client.logger.info(`Node version: ${blue(process.version)}`);
    this.client.logger.info(
      `Discord.js version: ${blue(
        dependencies["discord.js"].replace("^", "v")
      )}`
    );

    if (this.client.debug)
      this.client.logger.info(grey("Started in DEBUG MODE"));
